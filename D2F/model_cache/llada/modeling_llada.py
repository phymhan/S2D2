...
class RotaryEmbedding(nn.Module):
    ...
    def get_rotary_embedding_with_pos(self, device: torch.device, start_pos: int, end_pos: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if pos_cos.device != device:
            pos_cos = pos_cos.to(device)
            self.__cache["rope_pos_cos"] = pos_cos
        return pos_sin[:, :, start_pos:end_pos, :], pos_cos[:, :, start_pos:end_pos, :]
    ...
    def forward_with_pos(self, q: torch.Tensor, k: torch.Tensor, start_pos: int, end_pos: int) -> Tuple[torch.Tensor, torch.Tensor]:
        if self.config.rope_full_precision:
            q_, k_ = q.float(), k.float()
        else:
            q_, k_ = q, k
        ...
        with torch.autocast(q.device.type, enabled=False):
            pos_sin, pos_cos = self.get_rotary_embedding_with_pos(q_.device, start_pos, end_pos)
            pos_sin = pos_sin.type_as(q_)
            pos_cos = pos_cos.type_as(q_)
    ...
    def forward_with_pos_ids(self, q: torch.Tensor, k: torch.Tensor, position_ids: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        if self.config.rope_full_precision:
            q_, k_ = q.float(), k.float()
        else:
            q_, k_ = q, k
        ...
        with torch.autocast(q.device.type, enabled=False):
            ...
            if position_ids.shape[0] != B:
                if position_ids.shape[0] == 1:
                    ...
                    raise ValueError(f"position_ids batch mismatch: got {position_ids.shape[0]} vs B={B}")

            # Support:
            # - post_rope_cache path: q_len == k_len == current block length, but caller may pass full-length ids.
            # - pre_rope_cache path: k_len = past + current, q_len = current; caller should pass ids for k_len.
            ids_len = position_ids.shape[-1]
            if ids_len == k_len:
                k_pos_ids = position_ids
                q_pos_ids = position_ids[:, k_len - q_len : k_len]
            elif ids_len == q_len and k_len == q_len:
                # Common when rotating only the current block (no concatenated keys).
                q_pos_ids = position_ids
                k_pos_ids = position_ids
            elif ids_len > k_len:
                # Caller provided a longer sequence; use the suffix that aligns with the provided q/k tensors.
                k_pos_ids = position_ids[:, -k_len:]
                q_pos_ids = k_pos_ids[:, -q_len:]
            else:
                raise ValueError(
                    f"position_ids length mismatch: got {ids_len} but expected k_len={k_len} (or q_len={q_len} when q_len==k_len)"
                )

            # Ensure RoPE cache is large enough, then index sin/cos by position ids.
            max_pos = int(torch.max(k_pos_ids).item()) + 1
            pos_sin_all, pos_cos_all = self.get_rotary_embedding(max_pos, q_.device)  # (1, 1, max_pos, hs)
            pos_sin_all = pos_sin_all.type_as(q_)
            pos_cos_all = pos_cos_all.type_as(q_)

            # (max_pos, hs)
            sin_table = pos_sin_all[0, 0]
            cos_table = pos_cos_all[0, 0]

            # (B, 1, T, hs) -- broadcastable across heads
            q_sin = sin_table[q_pos_ids].unsqueeze(1)
            q_cos = cos_table[q_pos_ids].unsqueeze(1)
            k_sin = sin_table[k_pos_ids].unsqueeze(1)
            k_cos = cos_table[k_pos_ids].unsqueeze(1)

            q_ = self.apply_rotary_pos_emb(q_sin, q_cos, q_)
            k_ = self.apply_rotary_pos_emb(k_sin, k_cos, k_)
        return q_.type_as(q), k_.type_as(k)
...
class LLaDABlock(nn.Module):
    ...
    def _naive_sdpa(
        ...
        # Expand KV heads to match query heads if needed (GQA)
        assert k.size(1) == v.size(1)
        num_kv_heads = k.size(1)
        num_q_heads = q.size(1)
        if num_q_heads != num_kv_heads:
        ...
        att_weights = torch.matmul(q, k.transpose(2, 3)) * scale

        if attention_bias is not None:
            bias = self._cast_attn_bias(attention_bias, q.dtype)
            att_weights = att_weights + bias
            # ensure_finite_(att_weights, check_neg_inf=True, check_pos_inf=False)

        att_probs = F.softmax(att_weights, dim=-1, dtype=torch.float32).to(q.dtype)
    ...
    def attention(
        ...
        post_rope_cache = getattr(self.config, 'post_rope_cache', False)
        ...
        if self.config.rope:
            ...
            if post_rope_cache:
                # NOTE: we assume past_key and past_value are after RoPE, so only rotate new_key and q
                if position_ids is not None:
                    # assert position_ids has same length as new_key (unconcatenated k)
                    q_rot, k_rot = self.rotary_emb.forward_with_pos_ids(q, k, position_ids)
                else:
                    start_pos = 0 if layer_past is None else past_key.shape[-2]
                    end_pos = start_pos + query_len
                    q_rot, k_rot = self.rotary_emb.forward_with_pos(q, k, start_pos, end_pos)
                q_att = q_rot
                k_att = torch.cat([past_key, k_rot], dim=-2) if layer_past is not None else k_rot
                # k_att = k_att.contiguous()
                present = (k_att, v) if use_cache else None
            else:
                # Rotate for attention only; keep cached keys unrotated
                k = torch.cat((past_key, k), dim=-2) if layer_past is not None else k
                present = (k, v) if use_cache else None  # NOTE: k is pre-RoPE
                if position_ids is not None:
                    # assert position_ids has same length as (concatenated) k
                    q_att, k_att = self.rotary_emb.forward_with_pos_ids(q, k, position_ids)
                else:
                    q_att, k_att = self.rotary_emb(q, k)
        ...
        # att = self._naive_sdpa(q_att, k_att, v, attention_bias)

        # Re-assemble all head outputs side-by-side.
        att = att.transpose(1, 2).contiguous().view(B, T, C)

        # Apply output projection.
        # present already set appropriately above (pre- or post-RoPE)
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            att, cache = self.attention(
                q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, position_ids=position_ids
            )
    ...
    def forward(
        ...
        for block_idx, block in enumerate(self):
            ...
            if (
                ...
                # shape: (batch_size, seq_len, d_model)
                x, cache = block(
                    x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache, position_ids=position_ids
                )
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        # NOTE: Do not auto-generate `position_ids` here.
        # If callers want explicit RoPE positions (e.g., for non-contiguous layouts), they must pass
        # `position_ids` explicitly all the way down to attention.
            ...
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if attn_key_values is not None:
                    if update_kvcache:
                        ...
                        # print("True")
                        attn_key_values.append(cache)
                        # NOTE: (not update_kvcache) is never reached
                    #     if block_idx == 0:
                    #         print(f"update, update_kvcache: {update_kvcache}")
                    # else:
                    #     if block_idx == 0:
                    #         print(f"no update, update_kvcache: {update_kvcache}")
        else:
...
class LLaDAModelLM(PreTrainedModel):
    ...
    def forward(
        ...
        return CausalLMOutputWithPast(
            logits=logits,
            past_key_values=outputs.attn_key_values,  # NOTE: attn_key_values is the cached keys and values
    ...
    def generate_block_single(
        ...
        *,
        ...
        device = device or (input_ids.device if isinstance(input_ids, torch.Tensor) else torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        if dtype is None:
        ...
        while True:
            ...
            if len(block_states) - 1 < (max_new_tokens // block_size) and not eos_detected:
                ...
                if progress >= block_add_threshold:
                    ...
                    if new_start_pos + block_size <= max_length:
                        x_t = torch.cat([x_t, torch.full((1, block_size), mask_token_id, device=device, dtype=torch.long)], dim=1)
            ...
            attention_mask = _extract_attention_mask(full_attention_mask, process_start_pos, input_seq.shape[1], cache_length)
            # print(f"-> start_pos: {process_start_pos}, cache_length: {cache_length}, input_length: {input_seq.shape[1]}")

            # NOTE: to test position_ids forward pass
            # Build explicit (possibly non-contiguous) absolute position ids for RoPE.
            # For post_rope_cache=True we rotate only the current block (q/k are current-only),
            # so `position_ids` must match `input_seq` length.
            # For post_rope_cache=False we rotate over concatenated keys (past + current),
            # so `position_ids` should cover the full concatenated key length.
            # position_ids = None
            # cur_pos_ids = torch.arange(
            #     process_start_pos,
            #     process_start_pos + input_seq.shape[1],
            #     device=device,
            #     dtype=torch.long,
            # ).unsqueeze(0)
            # post_rope_cache = getattr(self.config, 'post_rope_cache', False)
            ...
            #         past_pos_ids = torch.arange(0, past_len, device=device, dtype=torch.long).unsqueeze(0)
            #         position_ids = torch.cat([past_pos_ids, cur_pos_ids], dim=1)
            #     else:
            #         position_ids = cur_pos_ids
            outputs = self(
                input_seq,
            ...
            # END NOTE: to test position_ids forward pass
            ...
            if return_forward_stats:
                ...
                included_active_block_ids = [
                    bid for bid in included_block_ids if block_states[bid]['state'] == 'active'
                ]
            ...
            for block_id, state in block_states.items():
                ...
                if selected_indices.numel() > 0:
                    ...
                    if eos_token_id is not None and (x0[selected_indices] == eos_token_id).any():
                        eos_detected = True
                    if return_forward_stats:
...
def _create_full_block_attention_mask(prompt_length: int, max_length: int, block_size: int, device=None, dtype=None):
    if dtype is None:
...
def _extract_attention_mask(full_mask: torch.Tensor, start_pos: int, input_length: int, cache_length: int) -> torch.Tensor:
...
def _construct_2L_attention_mask(extracted_mask: torch.Tensor, block_size: int) -> torch.Tensor:
    ...
    block_tokens = torch.ones((block_size, block_size), device=device, dtype=torch.bool)
    # lower_allowed = torch.tril(torch.ones((num_blocks, num_blocks), diagonal=-1, device=device, dtype=torch.bool))
    ...
    attn_2d = torch.full((num_rows * 2, cache_len + num_rows * 2), float("-inf"), device=device, dtype=dtype)
